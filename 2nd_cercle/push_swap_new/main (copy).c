...
/*   Updated: 2023/06/27 10:19:37 by vgiraudo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
void	ft_give_category(t_lst *lst)
{
	int		i;
	int		n;
	t_lst	*first;
	...
	while (i < 10)
	{
		n = (ft_lst_count_nocat(lst) / 5) + (ft_lst_count_nocat(lst) % 5 / 2);
		if (!n)
			n++;
		n *= i + 1;
		while (lst)
		{
			if (lst->pos / n == i && lst->category < 0)
				lst->category = i + 1;
			lst = lst->next;
		}
		lst = first;
...
int	main(int argc, char **argv)
{
	...
	if (!lst)
		return (printf("Error\n"));
	if (ft_hasdubs(lst))
		return (ft_free_lst(lst), printf("Error\n"));
