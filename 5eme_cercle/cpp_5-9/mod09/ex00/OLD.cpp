...
static void ft_check(std::string const & str, float const n)
{
	...
	std::string tmp = copyfrom(str, 5);
	std::string tmp2 = copyfrom(str, 8);
	fillWith(tmp, 2, 0);
	if (atoi(tmp.c_str()) < 1 || atoi(tmp.c_str()) > 12
		|| atoi(tmp2.c_str()) < 1 || atoi(tmp2.c_str()) > 31)
		throw std::out_of_range("Invalid date");
//	std::cout << "DEBUG:: " << tmp << std::endl;
//	std::cout << "DEBUG:: " << tmp2 << std::endl;
}
